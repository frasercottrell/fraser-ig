...
- `context/tone-of-voice.md` — how Fraser sounds.
...
If the repo isn't present (running elsewhere), the rules below are enough to work from.
...
5. **Run the linter:** `python3 scripts/voice-lint.py <draft>` (or pipe via `-`). Fix real flags by
...
- **Weave a credential right after the hook** — one casual line that earns the listen ("after 10,000+
  ads", "six years running an agency"). Proof said like a mate, not a CV.
